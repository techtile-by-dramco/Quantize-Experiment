...
DAC_FULLSCALE_RMS = 6.0         # Used only when DAC_DELTA=None: sets quantizer span ~ +/- (DAC_FULLSCALE_RMS * RMS)
...
ENABLE_DITHER = False           # True: add dither before quantization
...
# =============================================================================
#                   Dither + DAC Quantization helpers
# =============================================================================
...
def uniform_midrise_quantize_real(z: np.ndarray, b: int, delta: float) -> np.ndarray:
    """
    Symmetric midrise uniform quantizer Q_b(z) with L=2^b levels, step Δ.
    Levels: ℓ_i = Δ (i - L/2 + 1/2), i=0..L-1   (alpha handled outside if needed)
    Implemented with saturation to the outermost levels (τ_0=-inf, τ_L=+inf).
    """
    if b <= 0:
        raise ValueError("b must be >= 1")
    L = 1 << int(b)  # 2^b

    # Midrise quantization: Δ*(floor(z/Δ)+0.5)
    q = delta * (np.floor(z / delta) + 0.5)

    # Saturate to representable levels: ±Δ*(L/2 - 0.5)
    q_max = delta * (L / 2 - 0.5)
    q_min = -q_max
...
def dac_quantize_complex_midrise(
    x: np.ndarray,
    b: int,
    delta: float | None = None,
    power_norm: bool = True,
    fullscale_rms: float = 3.0,
    eps: float = 1e-12,
) -> np.ndarray:
    """
    Apply element-wise memoryless b-bit midrise uniform quantizer independently
    to real/imag parts:
        x_q = Q_b(Re{x}) + j Q_b(Im{x})

    If delta is None: choose Δ based on RMS(x) to avoid frequent saturation.
    power_norm=True: scale output to match input average power (implements alpha).
    """
    if x.size == 0:
    ...
    if delta is None:
        ...
        if rms <= eps:
            return x.astype(np.complex64)
        L = 1 << b
        # q_max = Δ*(L/2 - 0.5) ≈ fullscale_rms * rms  => Δ ≈ fullscale_rms*rms / (L/2 - 0.5)
        denom = (L / 2 - 0.5)
        delta = (fullscale_rms * rms) / max(denom, eps)

    re_q = uniform_midrise_quantize_real(np.real(x), b=b, delta=float(delta))
    im_q = uniform_midrise_quantize_real(np.imag(x), b=b, delta=float(delta))
    y = (re_q + 1j * im_q).astype(np.complex64)
    ...
    scale = np.sqrt(max(p_in, eps) / max(p_out, eps))
    return (scale * y).astype(np.complex64)
...
def tx_ref(usrp, tx_streamer, quit_event, w_u1, w_u2, phase_hw, start_time=None):
    ...
    try:
        while not quit_event.is_set():
            ...
            if ENABLE_DAC_QUANT:
                x = dac_quantize_complex_midrise(
                    x,
                    b=DAC_BITS,
                    delta=DAC_DELTA,
                    power_norm=DAC_POWER_NORM,
                    fullscale_rms=DAC_FULLSCALE_RMS,
                    eps=DAC_EPS,
                )
